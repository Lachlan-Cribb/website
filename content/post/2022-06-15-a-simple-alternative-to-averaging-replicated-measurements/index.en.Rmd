...
date: '2022-07-15'
slug: A-simple-alternative-to-averaging-replicated-exposure-measurements
categories: []
tags:
- bayes
- stan
...
Measuring exposure variables in replicates is a common fact in many medical research fields. For instance, blood biomarkers are often measured in duplicate. The purpose of measuring these variables in replicates is because their measurement is subject to considerable technical (i.e., coming from measurement instruments) and/or biological variability - and by performing repeated measurements we can hopefully measure and account for those forms of variation. In doing so, we get a better picture of the latent 'true' level of the exposure.


The most common approach that I see to handle replicated exposure measurements is simply to take their average and use that average as a predictor in subsequent regression models. This smooths away some noise and outliers - giving hopefully a more accurate picture of that true latent exposure level. This approach, while simple and intuitive, has some drawbacks. Notably, it leaves information on the table. The replicated measurements for each subject are treated independently - the measurements for one individual are not used to inform the estimation of others (there is no 'borrowing of information'). Consequently, this method might be subject to a lot of the problems that come with noisy measurement.
...
To see clearly how this method related to the next, I will write it out as a linear model. Note that the (point) estimates provided by this linear model are identical to simply averaging the replicates: 
...
\end{align*}
$$
Where $\text{x}_{ij}$ is the exposure measurement for the $j^{th}$ replicate for the $i^{th}$ participant, and $\mu_{it}$ represents the average exposure level for a given ID (participant). Because no information is shared between participants (each has their own intercept in the model), this model produces the so-called 'no pooling' estimate.
...
In the second approach, an overall global mean (intercept) is estimated and participant-level deviations from that overall mean are represented with an additional error term: 
...
d %>% 
  ...
  theme_default()
```
No pooling (average_x) estimates which are quite extreme are more aggresively pooled towards the overall mean than those which relatively more mean adjacent. 
...
Next, the coverage properties of the 95% confidence intervals are investigated to determine how often they actually cover the true regression coefficient across replicated datasets. Horizontal lines are added at 93.5% and 96.5% to cover a reasonable margin of error. 
...
At least in the context of this simulation study, using a hierarchical model to estimate the latent value of an exposure variable from replicates substantially outperforms the method of simply averaging replicates. And this comes at the cost of a very slight increase in analysis complexity. 

Importantly, both of these approaches share one problem. In each case, *model parameters* (estimates from either a linear model or hierarchical linear model) are, incorrectly, treated as if they were data. Treating them this way ignores the uncertainty associated with these parameters. Consequently, we might expect these models to produce overconfidet inferences. Nevertheless, this did not seem to have substantial consequences here, at least for the hierarchical approach. A more correct approach may be to instead estimate each of the parts of the model (estimate latent x and regress y on latent x) simultaneously in a more sophisticated hierarchical model. At this point, I am not sure of what (if any) advantages this might provide.

Lastly, in realistic scenarios, measurement error is probably not quite as well behaved as in this simulated scenario, in which it was sampled from a Gaussian distribution. That may neccesitate the use of other distributions, such as the student_t or lognormal. Missing data may also muddy the waters. I can't, however, imagine a scenario in which making full use of the sample data via a hierarchical model is not at least as good as the average approach. 

Let me know if you have any feedback/corrections/suggestions - I would gladly take criticism onboard. 
