...
res_mean <- broom::tidy(lm(y ~ mean_x, data = d), conf.int=T)[2,]
...
res_no_error <- broom::tidy(lm(y ~ x_no_error, data = d), conf.int=T)[2,]
...
This model seems to do much better than the two previous. The estimated regression coefficient is much closer (in fact almost exactly) to that of x_no_error. 
...
res_mean <- 
  res_mean %>% 
  mutate(term = recode(term, 'mean_x' = 'model 1 - mean x'))

res_bmod1 <- 
  samp$summary("beta", ~quantile(.x, c(0.025, 0.5, 0.975))) %>% 
  rename(term = variable, conf.low = `2.5%`, 
         estimate = `50%`, conf.high = `97.5%`) %>% 
  mutate(term = recode(term, 'beta' = 'model2 - flat priors'))

res_bmod2 <- 
  samp2$summary("beta", ~quantile(.x, c(0.025, 0.5, 0.975))) %>% 
  rename(term = variable, conf.low = `2.5%`, 
         estimate = `50%`, conf.high = `97.5%`) %>% 
  mutate(term = recode(term, 'beta' = 'model3 - hierarchical prior'))


bind_rows(
  res_bmod1,
  res_bmod2,
  res_mean %>% select(term, estimate, conf.low, conf.high),
  res_no_error %>% select(term, estimate, conf.low, conf.high),
) %>% 
